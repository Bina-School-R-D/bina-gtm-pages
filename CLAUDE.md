...
- `scripts/check-esa-static.mjs` enforces this for `/esa` against the built `dist/`, and runs inside `pnpm build`. A new variant page type gets the same treatment: assert on the built HTML, not the source.
...
|---|---|---|
| `/esa` | **Picker index only** — the hero, the five-step journey, and the state dropdown. It renders no state's answer itself: each option is a link to that state's own page at `/esa/<state>`, and choosing one navigates. Legacy `/esa?state=Arizona` deep links `location.replace` to the static URL (case-insensitive, compatibility only). | `src/data/esa-states.json`, synced from `bina-gtm/notes/ESA_State_Database.xlsx` (the team's ESA database — never invent numbers; update the xlsx-derived JSON instead) |
...
| `/esa/<state>` | **All 50 states, one static page each** (`/esa/arizona` … `/esa/wyoming`), built from one `[state].astro` + `getStaticPaths()`. Each page is the `/esa` tool with that state's answer server-rendered: the shared hero, the picker pre-selected, then `EsaStatePanel` — badge, three fact tiles, numbered bina/state steps, official links. Covers all five branches (`approved`, `approved-reimb`, `coming-soon`, `exploring`, and no-program states, which get an honest "no program you can use for bina yet"). Copy is the old client-side renderer's, unchanged. **This replaced the five paid-search landing pages** that used to own these URLs — `src/data/esa-landing.json` is now unreferenced; its `_openQuestions` still records the AZ/ID/UT/NH/WY status conflict and is worth reading before touching state statuses. | `src/data/esa-states.json` only (all program facts), via `src/lib/esa-states.js` — the shared content model both routes build from. Never hand-write a state page. |
