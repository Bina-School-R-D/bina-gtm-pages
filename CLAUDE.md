...
- **Tracking is a mirror of thebinaschool.com, set in `BaseLayout`** — GTM container `GTM-M2HWS5K` (CookieYes CMP + GA4 live inside it), PostHog project `phc_u3t5…` on `us.i.posthog.com`, and `public/attribution.js`, a port of the main site's file. All three use the main site's deferred loader (after load + 5s, or first interaction) and are host-gated to `*.thebinaschool.com` / `*.bina.school`, so local dev and previews stay out of the numbers. Re-extract from the live site if the main site's stack changes — don't invent a second setup.
