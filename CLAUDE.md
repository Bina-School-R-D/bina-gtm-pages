...
- **Primary CTA on every family-facing page is `Visit bina` → `https://thebinaschool.com`** (`/esa`, `/student-results` and `/reviews` all do this). We want the main site to take the enquiry; don't invent a page-specific CTA or point at a form unless the page *is* a campaign page. Campaign pages (`/book-adventure`) are the exception — they capture the lead themselves. `BaseLayout` keeps any `?ref=` from the current URL for 365 days and appends it to every `form.thebinaschool.com` link, mirroring the partner-attribution snippet on the main site (`bina-gtm/notes/partner-program-technical-plan.md` §4); `localStorage` is per-origin, so partner links routed through go.bina.school must carry `?ref=` on the go.bina.school URL itself.
