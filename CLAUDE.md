...
- **Tracking is a mirror of thebinaschool.com, set in `BaseLayout`** — GTM container `GTM-M2HWS5K` (CookieYes CMP + GA4 live inside it), PostHog, and `public/attribution.js`, a port of the main site's file. **PostHog is the one deliberate exception to parity**: it sends to a dedicated project for this domain — `go.bina.school`, us.posthog.com project `578193`, key `phc_D4SE…` — with session replay + console-log + performance capture enabled server-side, so this domain's traffic and replays are watched separately from the main site's project (`phc_u3t5…`). **PostHog now waits for resolved CookieYes analytics consent**: the local gate loads the SDK only when the getter reports literal `true`, rechecks after async loading, and stops capture/replay on revocation. GTM keeps its deferred loader (after load + 5s, or first interaction); attribution.js keeps its existing deferred script. This supersedes the eager replay behavior for this local consent candidate. All three are host-gated to `*.thebinaschool.com` / `*.bina.school`, so local dev and previews stay out of the numbers. Re-extract from the live site if the main site's stack changes — don't invent a second setup.
- **Attribution contract:** `attribution.js` stores the *first* `utm_*` / `gclid` / `ref` set it sees (localStorage, 365 days) and appends it to every `form.thebinaschool.com` HTTP(S) link and the observed `https://thebinaschool.com/` homepage destination (query/hash preserved), both on load and again in the click capture phase. Other main-site paths and hosts are excluded. `localStorage` is per-origin, so a first touch captured on go.bina.school is only carried across by those URL params — which is why the params, not a cookie, are the mechanism. Partner links that must attribute through go.bina.school still need `?ref=` on the go.bina.school URL itself.

- **Consent release evidence and limits:** `docs/go-consent-proposal.json` is a historical offline proposal, not an importable GTM export. Go-only CookieYes EU/UK targeting and the five-cookie inventory were published and read back on September 22, 2026. The shared GTM hostname lookup was saved in workspace 57; a local candidate running that draft passed controlled banner/consent checks, including same-document Analytics withdrawal and regrant. At that observation, the website fix and GTM draft had not been published. Forced banner preview does not prove live geographic routing, and PostHog bot filtering prevented delivery proof. Check current Pages and GTM versions before asserting release status; see `docs/go-consent-handoff.md` for the release sequence and limits.
...
- **Primary CTA on every family-facing page is `Visit bina` → `https://thebinaschool.com`** (`/esa`, `/esa/<state>`, `/student-results` and `/reviews` all do this). We want the main site to take the enquiry; don't invent a page-specific CTA or point at a form unless the page *is* a campaign page. Campaign pages are the exception — `/book-adventure` captures the lead itself. (The paid-search `/esa/<state>` landing pages sent every CTA to **`https://form.thebinaschool.com/new`** labelled **`Start your bina journey →`**, matching the main site's own conversion action — Akis, 2026-08-21. Those pages were replaced by the static state pages; reuse that CTA if paid-search landing pages come back.) `BaseLayout` keeps any `?ref=` from the current URL for 365 days and appends it to form links and the HTTPS main-site homepage, extending the partner-attribution snippet on the main site (`bina-gtm/notes/partner-program-technical-plan.md` §4); `localStorage` is per-origin, so partner links routed through go.bina.school must carry `?ref=` on the go.bina.school URL itself.
